...
//!
//! Conversely, the functionality in `reqwest_impersonate::blocking` must *not* be executed
...
//! [`reqwest_impersonate::Client`][crate::Client] instead. If the immediate context is only
...
//!
//! ```rust
//! # use reqwest_impersonate::{Error, Response};
//!
//! # fn run() -> Result<(), Error> {
//! let body = reqwest_impersonate::blocking::get("https://www.rust-lang.org")?
//!     .text()?;
//!
//! println!("body = {:?}", body);
//! # Ok(())
...
//! Reader, you can use the `reqwest_impersonate::blocking::Body::new()` constructor.
//!
//! ```rust
...
///
/// ```rust
/// # fn run() -> Result<(), reqwest_impersonate::Error> {
/// let body = reqwest_impersonate::blocking::get("https://www.rust-lang.org")?
///     .text()?;
/// # Ok(())
/// # }
